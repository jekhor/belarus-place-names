...
class Parser
	...
	def parse_line(string)
		if @state == :idle
			...
			if string =~ /@(Copy ([0-9] )?of )?Загал[1235]-?1? = (.+[^\s]) +СЕЛЬСАВЕТ\s*$/i
...
#        STDERR.puts "Нас. пунктаў: #{@our_stats[:total] - @prev}"
        @prev = @our_stats[:total]
			...
			if string =~ /@Загал[23] = (Населен?ныя пункты, адміністрацыйна падпарадкаваныя) (.+)$/
				@fields[:sovet] = $2
				STDERR.puts "#{$1} #{$2}"
				return
			end

			if string =~ /@Загал3 = Назвы населенн?ых пунктаў і парадыгмы іх склане.+я$/
				@state = :wait_for_table
				return
			end
      ...
      @header = true
		...
		if @state == :wait_for_cell_data
			if string =~ /@Table Text = (.*)$/ and (@header or string =~ /.*Тып паселішча.*/)
				@state = :wait_for_row
				return
			end

			if string =~ /@(Copy (.+ )?of )?Table( Body)? Text1? = (.*)$/
        STDERR.puts "WARNING: duplicate value: #{$4}" unless @fields[CELLS[@cell_num]].empty?
        if @fields[CELLS[@cell_num]].empty?
          @fields[CELLS[@cell_num]] = $4.gsub(/ +$/, '')
        else
          @fields[CELLS[@cell_num]] += ' ' + $4.gsub(/ +$/, '')
        end
				return
...
p = Parser.new("Магілёўская", STDOUT)
p.parse(STDIN)

...
class Parser
	...
	def parse_line(string)
		if @state == :idle
			if string =~ /@Загал1 = (.*)\s(РАЁН)$/
				@fields[:rayon] = $1.capitalize
			...
			if string =~ /@Загал5 = (.+) +СЕЛЬСАВЕТ.*$/
				@fields[:sovet] = $1.capitalize
				STDERR.puts "Знойдзены сельсавет '#{@fields[:sovet]}'"
				@our_stats[:sovets] += 1
				return
			end

			if string =~ /@Загал3 = .*Назвы пасёлкаў гарадскога тыпу і парадыгмы іх скланення.*/ or string =~ /@Загал3 = .*Назвы гарадоў( абласнога і)? раённага падпарадкавання.*$/
				if $1.nil?
					@fields[:sovet] = '<раён>'
				else
		...
		if @state == :wait_for_cell_data
			...
			if string =~ /@Table Body Text1? = (.*)$/
				@fields[CELLS[@cell_num]] = $1.gsub(/ +$/, '')
				return
			end
...
p = Parser.new("Гродзенская", STDOUT)
p.parse(STDIN)
